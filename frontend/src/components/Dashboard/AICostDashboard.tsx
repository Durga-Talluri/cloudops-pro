...
export default function AICostDashboard() {
  ...
  const fetchCostData = async (period: string = selectedPeriod) => {
    setIsLoading(true);
    try {
      const params = {
        time_range: period,
      ...
      const resAny: any = await apiService.get("/ai-cost/", params);
      if (resAny?.cost_data) {
        setCostData(
          (resAny.cost_data as any[]).map((d: any) => ({
            date: d.date,
            cost: d.cost,
      ...
      // Fetch optimizations separately if provided
      if (resAny?.optimization_suggestions) {
        setOptimizations(
          (resAny.optimization_suggestions as any[]).map((o: any) => ({
            id: String(o.id),
            title: o.title,
            description: o.description,
            potentialSavings: o.potential_savings ?? o.potentialSavings ?? 0,
            impact: o.impact ?? "medium",
            category: o.category ?? "General",
          }))
        );
      } else {
        // try optimizations endpoint
        try {
          const optResAny: any = await apiService.get(
            "/ai-cost/optimizations",
            { limit: 10 }
          );
          if (optResAny) {
            setOptimizations(
              (optResAny as any[]).map((o: any) => ({
                id: String(o.id),
                title: o.title,
                description: o.description,
                potentialSavings:
                  o.potential_savings ?? o.potentialSavings ?? 0,
                impact: o.impact ?? "medium",
                category: o.category ?? "General",
              }))
            );
          ...
          // ignore optimization fetch error
          console.warn("No optimization suggestions available", err);
        }
      }
    } catch (error) {
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div
              className={`flex items-center text-sm ${costChange >= 0 ? "text-red-600" : "text-green-600"}`}
            >
              {costChange >= 0 ? (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center mt-2 space-x-2">
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${getImpactColor(optimization.impact)}`}
                    >
