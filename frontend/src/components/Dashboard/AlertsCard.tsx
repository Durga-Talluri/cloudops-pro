...
export default function AlertsCard() {
  ...
  const fetchAlerts = async () => {
    setIsLoading(true);
    try {
      const resAny: any = await apiService.get("/alerts/", { limit: 50 });
      if (resAny?.alerts) {
        setAlerts(
          (resAny.alerts as any[]).map((a: any) => ({
            id: String(a.id),
            title: a.title,
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="text-right ml-2">
                <div
                  className={`inline-flex items-center px-2 py-1 text-xs rounded-full ${getStatusColor(alert.status)}`}
                >
